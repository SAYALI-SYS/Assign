...
const ChartsOverviewDemo: React.FC = () => {
  ...
  useEffect(() => {
    // Simulating fetching data from JSON (replace this with your actual fetch logic)
    const jsonData = [
        {
    ...
    // Transform JSON data into the format expected by BarChart component
    const transformedData: ChartData[] = jsonData.map(item => ({
      x: item.ts,
      y: item.machine_status === 0 ? 0 : item.machine_status === 1 ? 1 : 2,
      fill:
  ...
  return (
    <BarChart
      series={[{ data: chartData.map(item => item.y) }]} // Pass array of 'y' values
      xAxis={[{ data: chartData.map(item => item.x), scaleType: 'band' }]}
      yAxis={[{ scaleType: 'linear' }]} // Set up the y-axis as linear scale
      height={290}
      margin={{ top: 10, bottom: 30, left: 40, right: 10 }}
      seriesOptions={{
        colorAccessor: d => chartData[d.index].fill, // Use fill property as color accessor
      }}
    />
  );
};
